...
let paths = () => {
	return new Promise(resolve=>{
		...
		for (let i in files) {
			fs.stat(("D:&programiranje&USBWebserver v8.6&root&OBSKC-Raspored-Sati&converter&data&xlsx&"+files[i]).replace(/&/g,"/"), (err,stats)=>{
...
let init = async function(){
	let paths = await paths();
	let test = readABook(paths);
}
init();
