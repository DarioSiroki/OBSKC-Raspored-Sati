  ...
  for (let i = 1; i < data.length; i++) {
    for (let ii = 1; ii < data[i].length; ii++) {
      ...
      for (let iii = 0; iii < filterWords.length; iii++) {
        if (
          typeof podatak === "object" &&
          podatak.hasOwnProperty("name") &&
          podatak.name
        ) {
          if (
            typeof podatak.name != "string" ||
            podatak.name
              .toUpperCase()
              .indexOf(filterWords[iii].toUpperCase()) !== -1
          ) {
            podatak.name = "";
          }
        }
...
let writeVersions = () => {
  ...
  fs.readdirSync(writeDir).forEach((file) => {
    if (
      file.toLowerCase().indexOf("json") !== -1 &&
      file.indexOf("versions") == -1
    ) {
      versions.push(file);
    }
  });
...
fs.readdirSync(scanDir).forEach((file) => {
  fs.stat(scanDir + file, (err, stats) => {
    ...
    if (seconds < 172800) {
      //172800 = 2 dana, ako je file modified prije vise od 2 dana ne konvertiraj ga
      paths.push(file);
    }
  });
});
...
setTimeout(() => {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    var info = {
      ...
      datum: workbook.A2.v,
      prijeposlije:
        workbook.A1.v.search("POSLIJE") === -1 ? "PRIJE" : "POSLIJE",
      napomena: "",
    };
